...
/* exceptions handler                */
extern void t_divide_handler(void);      /* 0 */
extern void t_debug_handler(void);       /* 1 */
extern void t_nmi_handler(void);         /* 2 */
extern void t_brkpt_handler(void);       /* 3 */
extern void t_oflow_handler(void);       /* 4 */
extern void t_bound_handler(void);       /* 5 */
extern void t_illop_handler(void);       /* 6 */
extern void t_device_handler(void);      /* 7 */
extern void t_dblflt_handler(void);      /* 8 */
extern void t_tss_handler(void);         /* 10 */
extern void t_segnp_handler(void);       /* 11 */
extern void t_stack_handler(void);       /* 12 */
extern void t_gpflt_handler(void);       /* 13 */
extern void t_pgflt_handler(void);       /* 14 */
extern void t_fperr_handler(void);       /* 16 */
extern void t_align_handler(void);       /* 17 */
extern void t_mchk_handler(void);        /* 18 */
extern void t_simderr_handler(void);     /* 19 */
extern void t_syscall_handler(void);     /* 48 */
//void sysenter_handler(void);

/* hardware interrupts handler       */
extern void irq_timer_handler(void);   /* 0 */
extern void irq_kbd_handler(void);     /* 1 */
extern void irq_2_handler(void);       /* 2 */
extern void irq_3_handler(void);       /* 3 */
extern void irq_serial_handler(void);  /* 4 */
extern void irq_5_handler(void);       /* 5 */
extern void irq_6_handler(void);       /* 6 */
extern void irq_spurious_handler(void);/* 7 */
extern void irq_8_handler(void);       /* 8 */
extern void irq_9_handler(void);       /* 9 */
extern void irq_10_handler(void);      /* 10 */
extern void irq_11_handler(void);      /* 11 */
extern void irq_12_handler(void);      /* 12 */
extern void irq_13_handler(void);      /* 13 */
extern void irq_ide_handler(void);     /* 14 */
extern void irq_15_handler(void);      /* 15 */
...
trap_init(void) {
  ...
  SETGATE(idt[T_OFLOW], 0, GD_KT, t_oflow_handler, 3);
  SETGATE(idt[T_BOUND], 0, GD_KT, t_bound_handler, 3);
...
trap_init_percpu(void) {
  ...
  cpu_ts->ts_esp0 = KSTACKTOP - i * (KSTKSIZE + KSTKGAP);
  cpu_ts->ts_ss0 = GD_KD;
  cpu_ts->ts_iomb = sizeof(struct Taskstate);

  gdt[(GD_TSS0 >> 3) + i] = SEG16(STS_T32A, (uint32_t) cpu_ts, sizeof(struct Taskstate), 0);
...
page_fault_handler(struct Trapframe *tf) {
  ...
  if (!(tf->tf_cs & 0x3))
    panic("trap.c page_fault_handler: kernel page fault");
  ...
  if (curenv->env_pgfault_upcall) {
    ...
    struct UTrapframe *utf = (struct UTrapframe *)
        ((tf->tf_esp >= UXSTACKTOP - PGSIZE && tf->tf_esp <= UXSTACKTOP - 1 ?
...
switch_and_trap(struct Trapframe *frame) {
  ...
  if ((frame->tf_cs & 3) == 3) {
    ...
    int cpunum = (KERNBASE - (uintptr_t) (frame - 1)) / (KSTKSIZE + KSTKGAP);
    struct Env *e = cpus[cpunum].cpu_env;
